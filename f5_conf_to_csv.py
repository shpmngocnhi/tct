...
def write_csv(conn, query, filename):
    ...
    db_print.to_csv(filename, index=False, encoding="utf-8-sig")
...
def convert_f5_conf_to_csv(f5_conf):
    # Default Values
    F5_db = 'F5.sqlite3'
    nodes={}
    # Initialize Database
    ...
    File = open(f5_conf, "r", encoding='utf-8')
    ...
    for idx, val in enumerate(lines):
        if val.startswith("ltm node"):
            ...
            for i, v in enumerate(stanza):
                ...
                elif "pool" in v and 'source-address-translation' not in stanza[i - 1]:
    ...
    conn.commit()
    f5_conf = f5_conf.replace('.conf', '')
    # Write Tables to CSV
    write_csv(conn,"SELECT * FROM Nodes", f"{f5_conf}-f5-nodes.csv")
    write_csv(conn,"SELECT * FROM Pools", f"{f5_conf}-f5-pools.csv")
    write_csv(conn,"SELECT * FROM VirtualAddresses", f"{f5_conf}-f5-virtualaddresses.csv")
    write_csv(conn,"SELECT * FROM VirtualServers", f"{f5_conf}-f5-virtualservers.csv")
    write_csv(conn,"SELECT * FROM Monitors", f"{f5_conf}-f5-monitors.csv")
    write_csv(conn,"SELECT * FROM Profiles", f"{f5_conf}-f5-profiles.csv")
    write_csv(conn,"SELECT * FROM Persistence", f"{f5_conf}-f5-persistence.csv")
    write_csv(conn,"SELECT * FROM Rules", f"{f5_conf}-f5-rules.csv")
    write_csv(conn,"SELECT VirtualServers.Name as 'VirtualServer', Pools.Name as 'Pool', Pools.Member_Name as 'Pool Member', Pools.Member_Address, VirtualServers.IP_Protocol as 'Protocol', VirtualServers.Profiles, VirtualServers.Rules, Pools.Monitor FROM Pools LEFT JOIN VirtualServers ON Pools.Name = VirtualServers.Pool", f"{f5_conf}-f5-overview.csv")
